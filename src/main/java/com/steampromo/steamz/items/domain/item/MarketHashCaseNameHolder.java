package com.steampromo.steamz.items.domain.item;


import java.util.Arrays;
import java.util.List;

public class MarketHashCaseNameHolder {

    private static final List<String> MARKET_HASH_NAMES = Arrays.asList(
            "Chroma%20Case",
            "Chroma%202%20Case",
            "Chroma%203%20Case",
            "CS:GO%20Weapon%20Case",
            "CS:GO%20Weapon%20Case%202",
            "CS:GO%20Weapon%20Case%203",
            "Clutch%20Case",
            "CS20%20Case"
//            "Danger%20Zone%20Case",
//            "Dreams%20&%20Nightmares%20Case",
//            "eSports%202013%20Case",
//            "eSports%202013%20Winter%20Case",
//            "eSports%202014%20Summer%20Case",
//            "Falchion%20Case",
//            "Fracture%20Case",
//            "Gamma%20Case",
//            "Gamma%202%20Case",
//            "Glove%20Case",
//            "Horizon%20Case",
//            "Huntsman%20Weapon%20Case",
//            "Operation%20Bravo%20Case",
//            "Operation%20Breakout%20Case",
//            "Operation%20Hydra%20Case",
//            "Operation%20Phoenix%20Weapon%20Case",
//            "Operation%20Riptide%20Case",
//            "Operation%20Vanguard%20Weapon%20Case",
//            "Operation%20Wildfire%20Case",
//            "Prisma%20Case",
//            "Prisma%202%20Case",
//            "Revolver%20Case",
//            "Revolution%20Case",
//            "Shadow%20Case",
//            "Shattered%20Web%20Case",
//            "Snakebite%20Case",
//            "Spectrum%20Case",
//            "Spectrum%202%20Case",
//            "Winter%20Offensive%20Weapon%20Case"

    );

    public static List<String> getMarketHashNames() {
        return MARKET_HASH_NAMES;
    }
}
